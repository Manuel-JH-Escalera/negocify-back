...
router.get('/', async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: 'Error fetching tipos de venta', error: error.message });
    }
});
...
router.get('/:id', protect, async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: 'Error al obtener tipo de venta', error: error.message });
    }
  });
...
router.post('/', protect, async (req, res) => {
    try {
      ...
      if (!nombre || comision === undefined) {
        return res.status(400).json({ message: 'Nombre y comisión son campos requeridos' });
      }
      ...
      return res.status(500).json({ message: 'Error al crear tipo de venta', error: error.message });
    }
  });
...
router.put('/:id', protect, async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: 'Error al actualizar tipo de venta', error: error.message });
    }
  });
...
router.delete('/:id', protect, async (req, res) => {
    try {
      ...
      return res.status(200).json({ message: 'Tipo de venta eliminado correctamente' });
    } catch (error) {
      console.error('Error al eliminar tipo de venta:', error);
      return res.status(500).json({ message: 'Error al eliminar tipo de venta', error: error.message });
    }
  });
